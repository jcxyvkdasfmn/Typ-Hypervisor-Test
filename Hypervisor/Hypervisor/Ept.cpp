...
InitEptp()
{
    ...
    PVOID GuestMemory = ExAllocatePool2(POOL_FLAG_NON_PAGED, PagesToAllocate * PAGE_SIZE, POOLTAG);

    g_VirtualGuestMemoryAddress = (UINT64)GuestMemory;
    ...
    for (size_t i = 0; i < PagesToAllocate; i++)
    {
        ...
        UINT64 num = (g_VirtualGuestMemoryAddress + (i * PAGE_SIZE)) / PAGE_SIZE;
        EptPt[i].Fields.PhysicalAddress = (VirtualAddressToPhysicalAddress((PVOID)num));
    ...
    EptPd->Fields.PhysicalAddress = (VirtualAddressToPhysicalAddress(EptPt) / PAGE_SIZE);
    EptPd->Fields.Read = 1;
    ...
    EptPdpt->Fields.PhysicalAddress = (VirtualAddressToPhysicalAddress(EptPd) / PAGE_SIZE);
    ...
    EptPml4->Fields.PhysicalAddress = (VirtualAddressToPhysicalAddress(EptPdpt) / PAGE_SIZE);
    ...
    EPTPointer->Fields.PML4Address = (VirtualAddressToPhysicalAddress(EptPml4) / PAGE_SIZE);
