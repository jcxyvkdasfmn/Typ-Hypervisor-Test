...
bool
TerminateVMMInitState(PVMM_INIT_STATE VMMInitState)
{
	FreeVmxon(VMMInitState);

	FreeVmcs(VMMInitState);

	FreeMsrBitmap(VMMInitState);

	FreeVmmStack(VMMInitState);
...
bool StopVmx(PVMM_INIT_STATE InitState) {
	...
	TerminateVMMInitState(InitState);
...
extern "C" void Start(PVOID GuestRsp) {
	...
	PVMM_INIT_STATE InitState = NULL;

	//To do: save Guest Rsp
	InitState = &g_VMMInitState[KeGetCurrentProcessorNumber()];

	SetupVmcs(InitState, GuestRsp);



	int Status = __vmx_vmlaunch();

	DBG_LOG("Vmx Launch failed with Status: %d", Status);

	size_t ErrorCode = 0;

	__vmx_vmread(VM_INSTRUCTION_ERROR, &ErrorCode);

	DBG_LOG("VMLAUNCH Error : 0x%llx\n", ErrorCode);
