...
PEPTP InitEptp()
{
    ...
    if (!GuestMemory)
    {
        DBG_LOG("Couldn't allocate Guest Memory");
        return false;
    }
    ...
    for (size_t i = 0; i < PagesToAllocate; i++)
    {
        ...
        UINT64 physaddruint = GuestMemory + (i * PAGE_SIZE);
        UINT64  physaddr = VirtualAddressToPhysicalAddress((PVOID)physaddruint);
        EptPt[i].Fields.PhysicalAddress = physaddr / PAGE_SIZE;
    ...
    DBG_LOG("Extended Page Table Pointer allocated at %llx\n", EPTPointer);
