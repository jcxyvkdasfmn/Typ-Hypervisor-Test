...
AllocateVmxonRegion(PVMM_INIT_STATE InitState)
{
	...
	DBG_LOG("MSR_IA32_VMX_BASIC (MSR 0x480) Revision Identifier %llx", basic.Fields.RevisionIdentifier);
...
bool AllocateVmcsRegion(PVMM_INIT_STATE InitState)
{
	...
	DBG_LOG("Revision Identifier (MSR_IA32_VMX_BASIC - MSR 0x480) : 0x%x", VmxBasicMsr.Fields.RevisionIdentifier);
...
bool AllocateMsrBitmap(PVMM_INIT_STATE InitState)
{
	...
	InitState->PhysicalMsrBitmap = MmGetPhysicalAddress(InitState->MsrBitmap);

	DBG_LOG("Msr Bitmap Virtual Address : 0x%llx", InitState->MsrBitmap);
	DBG_LOG("Msr Bitmap Physical Address : 0x%llx", InitState->PhysicalMsrBitmap.QuadPart);
...
bool
ClearVmcsState(PVMM_INIT_STATE Initstate)
{
	// UUUPPSSSIII Daisy Dont pass like that: unsigned long long ptr * = (unsigned long long *)Initstate->PhysicalVMCSRegion;
	unsigned long long  ptr = (unsigned long long)Initstate->PhysicalVMCSRegion;

	DBG_LOG("Address of where the physical Address of Vmcs is stored: 0x%llu", ptr);

	int status = __vmx_vmclear(&ptr);

	DBG_LOG("Vmclear Status is : %d", status);
	if (status)
	{
...
bool
LoadVmcs(PVMM_INIT_STATE Initstate)
{
	unsigned long long ptr = (unsigned long long)Initstate->PhysicalVMCSRegion;
	// UUUPPSSSIII Daisy Dont pass like that: unsigned long long ptr * = (unsigned long long *)Initstate->PhysicalVMCSRegion;
	DBG_LOG("Address of where the physical Address of Vmcs is stored: 0x%llu", ptr);

	int status = __vmx_vmptrld(&ptr);

	DBG_LOG("Vmptrld Status is : %d", status);
	if (status)
	{
...
bool SetupVmcs(PVMM_INIT_STATE InitState, PVOID GuestStack)
{
	UINT64                  GdtBase = 0;
	SEGMENT_SELECTOR    SegmentSelector = { 0 };
	IA32_VMX_BASIC_MSR VmxBasicMsr = { 0 };
	...
	*/
	...
	__vmx_vmwrite(MSR_BITMAP, (size_t)InitState->PhysicalMsrBitmap.LowPart);
	__vmx_vmwrite(MSR_BITMAP_HIGH, (size_t)InitState->PhysicalMsrBitmap.HighPart);
